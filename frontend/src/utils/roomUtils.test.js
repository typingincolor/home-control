...
import { getScenesForRoom, buildRoomHierarchy, calculateRoomStats } from './roomUtils';

describe('roomUtils', () => {
  describe('getScenesForRoom', () => {
    it('should return empty array when no scenes data', () => {
      const result = getScenesForRoom(null, 'room-1');
      expect(result).toEqual([]);
    });

    it('should return empty array when scenes data has no data property', () => {
      const result = getScenesForRoom({}, 'room-1');
      expect(result).toEqual([]);
    });
    ...
    it('should filter scenes by room UUID', () => {
      ...
      const result = getScenesForRoom(scenesData, 'room-1');
      expect(result).toHaveLength(2);
      expect(result[0].uuid).toBe('scene-1');
      expect(result[1].uuid).toBe('scene-3');
    });
  ...
  describe('buildRoomHierarchy', () => {
    it('should return null when missing data', () => {
      expect(buildRoomHierarchy(null, null, null)).toBeNull();
      expect(buildRoomHierarchy({}, {}, {})).toBeNull();
      expect(buildRoomHierarchy({ data: [] }, null, null)).toBeNull();
    });
    ...
    it('should build room to light mapping via devices', () => {
      ...
      const result = buildRoomHierarchy(lights, rooms, devices);
  ...
  describe('calculateRoomStats', () => {
    it('should return zeros for empty array', () => {
      const result = calculateRoomStats([]);
      expect(result).toEqual({
        lightsOnCount: 0,
        totalLights: 0,
        averageBrightness: 0
    ...
    it('should return zeros for null input', () => {
      const result = calculateRoomStats(null);
      expect(result).toEqual({
        lightsOnCount: 0,
        totalLights: 0,
        averageBrightness: 0
    ...
    it('should count lights correctly', () => {
      ...
      const result = calculateRoomStats(lights);
